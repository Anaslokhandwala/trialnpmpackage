...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        if(accuraErrorCode["CameraScreen_Color"] as! String == ""){
            ...
            viewStatusBar.backgroundColor = UIColor(hexString: accuraErrorCode["CameraScreen_Color"] as! String) //UIColor(red: 231.0 / 255.0, green: 52.0 / 255.0, blue: 74.0 / 255.0, alpha: 1.0)
            viewNavigationBar.backgroundColor = UIColor(hexString: accuraErrorCode["CameraScreen_Color"] as! String) //UIColor(red: 231.0 / 255.0, green: 52.0 / 255.0, blue: 74.0 / 255.0, alpha: 1.0)
        }
        ...
        //        frameImg = AccuraKyc.getImageFromUri(path: accuraErrorCode["setCameraFrame"] as! String)
...
//        var backData:[String: Any] = [:]
        ...
        if let frontUri = AccuraKyc.getImageUri(img: imageNumberPlate, name: nil) {
        ...
        callBack!([NSNull(),AccuraKyc.convertJSONString(results:results)])
        self.dismiss(animated: true)
    ...
    func resultData(_ resultmodel: ResultModel!) {
        ...
        if let faceUri = AccuraKyc.getImageUri(img: resultmodel.faceImage, name: nil) {
            results["face"] = faceUri
        }
        if let frontUri = AccuraKyc.getImageUri(img: resultmodel.frontSideImage, name: nil) {
            results["front_img"] = frontUri
        }
        if let backUri = AccuraKyc.getImageUri(img: resultmodel.backSideImage, name: nil) {
            ...
            if let k = data.key as? String {
                if k == "Signature" {
                    if let base64img = data.value as? String {
                        ...
                        if let sigUri = AccuraKyc.getImageUri(img: decodedimage, name: nil) {
...
//        var backData:[String: Any] = [:]
        ...
        if FrontImage != nil {
            ...
            if let frontUri = AccuraKyc.getImageUri(img: FrontImage!, name: nil) {
                results["front_img"] = frontUri
            }
        }
        if BackSideImage != nil {
            //            imgViewBack = BackSideImage
            if let frontUri = AccuraKyc.getImageUri(img: BackSideImage!, name: nil) {
                results["back_img"] = frontUri
            }
        }
        if FaceImage != nil {
            if let frontUri = AccuraKyc.getImageUri(img: FaceImage!, name: nil) {
    ...
    func recognizeSucceed(_ scanedInfo: NSMutableDictionary!, recType: RecType, bRecDone: Bool, bFaceReplace: Bool, bMrzFirst: Bool, photoImage: UIImage, docFrontImage: UIImage!, docbackImage: UIImage!) {
        ...
        if(bMrzFirst)
        {
            if isBackSide!{
                if docbackImage != nil {
                if let frontUri = AccuraKyc.getImageUri(img: docbackImage, name: nil) {
                    results["back_img"] = frontUri
                    }
                }
                documentImage = docbackImage
                if(docFrontImage != nil) {
                    if let frontUri = AccuraKyc.getImageUri(img: docFrontImage, name: nil) {
                        results["front_img"] = frontUri
                    }
            ...
            if let frontUri = AccuraKyc.getImageUri(img: photoImage, name: nil){
                results["face"] = frontUri
            }
            let kK = shareScanningListing.allKeys
            let vV = shareScanningListing.allValues
...
//        let backData:[String: Any] = [:]
        if let frontUri = AccuraKyc.getImageUri(img: bankCardImage, name: nil) {
            results["front_img"] = frontUri
        }
